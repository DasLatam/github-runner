...
async function runVisit(item, browser) {
  ...
  try {
    ...
    // Si aparece un overlay, lo intentamos sacar
    try {
    ...
    // Espera adicional de carga real
    try {
      await page.waitForLoadState('load', { timeout: 15000 });
    } catch {}

    // Interacción mínima “humana”
    try {
    ...
    // Intento explícito de aceptar cookies si siguen visibles
    await dismissCommonBanners(page);

    // Observación principal
    await page.waitForTimeout(60000);

    // Drenaje final para que terminen requests pendientes
